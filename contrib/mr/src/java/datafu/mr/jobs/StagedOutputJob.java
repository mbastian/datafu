...
/**
 * A derivation of {@link Job} that stages its output in another location and
 * only moves it to the final destination if the job completes successfully. It
 * also outputs a counters file to the file system that contains counters
 * fetched from Hadoop and other task statistics.
 */
public class StagedOutputJob extends Job implements Callable<Boolean> {
	...
	/**
	 * Creates a job which using a temporary staging location for the output
	 * data. The data is only copied to the final output directory on successful
	 * completion of the job. This prevents existing output data from being
	 * overwritten unless the job completes successfully.
	 * 
	 * @param conf
	 *            configuration
	 * @param jobName
	 ...
	 * @return job
	 */
	public static StagedOutputJob createStagedJob(Configuration conf,
			String jobName, List<String> inputPaths, String stagingLocation,
			String outputPath, final Logger log) {
		...
		try {
			...
			FileInputFormat.setInputPathFilter(retVal,
					HiddenFilePathFilter.class);
		} catch (IOException e) {
	 ...
	 * @throws IOException
	 */
	public StagedOutputJob(Configuration conf, String stagingPrefix, Logger log)
			throws IOException {
		super(conf);
	...
	/**
	 * Gets path to store the counters. If this is not set then by default the
	 * counters will be stored in the output directory.
	 * 
	...
	/**
	 * Sets path to store the counters. If this is not set then by default the
	 * counters will be stored in the output directory.
	 * 
	 * @param path
	...
	public Boolean call() throws Exception {
		try {
			...
			if (getJobID() != null) {
				jobId = String.format("job_%s_%d",
						getJobID().getJtIdentifier(), getJobID().getId());
			}

			if (success) {
				_log.info(String.format(
						"Job %s with ID %s succeeded! Tracking URL: %s",
						getJobName(), jobId, this.getTrackingURL()));
			} else {
				_log.error(String.format(
						"Job %s with ID %s failed! Tracking URL: %s",
						getJobName(), jobId, this.getTrackingURL()));
			}
	...
	/**
	 * Run the job and wait for it to complete. Output will be temporarily
	 * stored under the staging path. If the job is successful it will be moved
	...
	public boolean waitForCompletion(boolean verbose) throws IOException,
		...
		final Path stagedPath = new Path(String.format("%s/%s/staged",
				_stagingPrefix, System.currentTimeMillis()));
		...
		if (retVal) {
			...
			_log.info(String.format("Deleting data at old path[%s]",
					actualOutputPath));
			fs.delete(actualOutputPath, true);

			_log.info(String.format(
					"Moving from staged path[%s] to final resting place[%s]",
					stagedPath, actualOutputPath));
	...
	private static Class<?> getCallersClass() {
		...
		for (StackTraceElement element : stack) {
			if (foundSelf
					&& !StagedOutputJob.class.getName().equals(
			...
			} else if (StagedOutputJob.class.getName().equals(
					element.getClassName())
	...
	/**
	 * Writes Hadoop counters and other task statistics to a file in the file
	 * system.
	 * 
	 * @param fs
	 * @throws IOException
	...
	private void writeCounters(final FileSystem fs) throws IOException {
		...
		BufferedOutputStream buffer = new BufferedOutputStream(counterStream,
				256 * 1024);
		...
		for (String groupName : getCounters().getGroupNames()) {
			for (Counter counter : getCounters().getGroup(groupName)) {
				writeAndLog(
						writer,
						String.format("%s=%d", counter.getName(),
								counter.getValue()));
			}
		}
		...
		org.apache.hadoop.mapred.JobID oldJobId = new org.apache.hadoop.mapred.JobID(
		...
		if (mapReports.length > 0) {
			...
			for (TaskReport report : mapReports) {
				...
				mapStats.addValue(report.getFinishTime()
						- report.getStartTime());
			}
		} else {
		...
		if (reduceReports.length > 0) {
			...
			for (TaskReport report : reduceReports) {
				...
				reduceStats.addValue(report.getFinishTime()
						- report.getStartTime());
			}
		} else {
		...
		for (TaskCompletionEvent event : getTaskCompletionEvents(jobClient,
				oldJobId)) {
			String type = taskIdToType.get(event.getTaskAttemptId().getTaskID()
					.toString());
		...
		if (success) {
			writeAndLog(writer,
					String.format("SETUP_START_TIME_MS=%d", setupStart));
			writeAndLog(writer,
					String.format("CLEANUP_FINISH_TIME_MS=%d", cleanupFinish));
			writeAndLog(writer, String.format("COMPLETE_WALL_CLOCK_TIME_MS=%d",
					cleanupFinish - setupStart));

			writeAndLog(writer,
					String.format("MAP_REDUCE_START_TIME_MS=%d", minStart));
			writeAndLog(writer,
					String.format("MAP_REDUCE_FINISH_TIME_MS=%d", maxFinish));
			writeAndLog(
					writer,
					String.format("MAP_REDUCE_WALL_CLOCK_TIME_MS=%d", maxFinish
							- minStart));

			writeAndLog(writer,
					String.format("MAP_TOTAL_TASKS=%d", (long) mapStats.getN()));
			writeAndLog(
					writer,
					String.format("MAP_MAX_TIME_MS=%d",
							(long) mapStats.getMax()));
			writeAndLog(
					writer,
					String.format("MAP_MIN_TIME_MS=%d",
							(long) mapStats.getMin()));
			writeAndLog(
					writer,
					String.format("MAP_AVG_TIME_MS=%d",
							(long) mapStats.getMean()));
			writeAndLog(
					writer,
					String.format("MAP_STD_TIME_MS=%d",
							(long) mapStats.getStandardDeviation()));
			writeAndLog(
					writer,
					String.format("MAP_SUM_TIME_MS=%d",
							(long) mapStats.getSum()));

			writeAndLog(
					writer,
					String.format("REDUCE_TOTAL_TASKS=%d",
							(long) reduceStats.getN()));
			writeAndLog(
					writer,
					String.format("REDUCE_MAX_TIME_MS=%d",
							(long) reduceStats.getMax()));
			writeAndLog(
					writer,
					String.format("REDUCE_MIN_TIME_MS=%d",
							(long) reduceStats.getMin()));
			writeAndLog(
					writer,
					String.format("REDUCE_AVG_TIME_MS=%d",
							(long) reduceStats.getMean()));
			writeAndLog(
					writer,
					String.format("REDUCE_STD_TIME_MS=%d",
							(long) reduceStats.getStandardDeviation()));
			writeAndLog(
					writer,
					String.format("REDUCE_SUM_TIME_MS=%d",
							(long) reduceStats.getSum()));

			writeAndLog(
					writer,
					String.format(
							"MAP_REDUCE_SUM_TIME_MS=%d",
							(long) mapStats.getSum()
									+ (long) reduceStats.getSum()));

			for (Map.Entry<String, Integer> attemptStat : attemptStats
					.entrySet()) {
				writeAndLog(writer, String.format("%s=%d",
						attemptStat.getKey(), attemptStat.getValue()));
			}
		}
	 ...
	 * @throws IOException
	 */
	private List<TaskCompletionEvent> getTaskCompletionEvents(
			JobClient jobClient, org.apache.hadoop.mapred.JobID jobId)
			throws IOException {
		...
		try {
			...
			if (jobSubmitClient != null) {
				getTaskCompletionEventsMethod = jobSubmitClient.getClass()
						.getDeclaredMethod("getTaskCompletionEvents",
								org.apache.hadoop.mapred.JobID.class,
								int.class, int.class);
		...
		while (true) {
			...
			if (getTaskCompletionEventsMethod != null) {
				try {
					...
					currentEvents = (TaskCompletionEvent[]) getTaskCompletionEventsMethod
							.invoke(jobSubmitClient, jobId, index, 250);
				} catch (IllegalArgumentException e) {
					_log.error(
							"Failed to call getTaskCompletionEventsMethod via reflection, switching to slow direct method",
							e);
	...
	private void writeAndLog(OutputStreamWriter writer, String line)
			throws IOException {
		writer.append(line);
		writer.append("\n");
		_log.info(line);
