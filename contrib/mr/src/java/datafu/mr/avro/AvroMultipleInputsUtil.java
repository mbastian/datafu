...
/**
 * Helper methods for dealing with multiple Avro input schemas. A mapping is
 * stored in the configuration that maps each input path to its corresponding
 * schema. Methods in this class help with loading and storing these schema
 * mappings.
 * 
...
public class AvroMultipleInputsUtil {
	private static final Logger _log = Logger
			.getLogger(AvroMultipleInputsUtil.class);
	 ...
	 * @return schema
	 */
	public static Schema getInputKeySchemaForSplit(Configuration conf,
			InputSplit split) {
		...
		if (schemas != null) {
			for (String key : JSONObject.getNames(schemas)) {
				...
				if (path.startsWith(key)) {
					try {
						schema = new Schema.Parser().parse(schemas
								.getString(key));
						_log.info("Input schema found: "
	 ...
	 */
	public static void setInputKeySchemaForPath(Job job, Schema schema,
			String path) {
		JSONObject schemas;
		try {
	 ...
	 */
	private static JSONObject getInputSchemas(Configuration conf)
			throws JSONException {
		JSONObject schemas;
